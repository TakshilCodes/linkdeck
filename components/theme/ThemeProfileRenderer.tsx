...
/** iPhone 14 logical portrait (390 x 844); dashboard preview matches real phone canvas. */
...
export default function ThemeProfileRenderer({
  ...
  const { patternColor, blurValue, profileColor } = getThemeTokens(theme);
  const orderedSections = sections?.filter((section) => section.links.length > 0);

  const innerScrollClass =
    layout === "embed"
      ? "relative z-10 flex h-full min-h-0 flex-col overflow-y-auto overflow-x-hidden scrollbar-hide px-7 pb-8 pt-11"
        ...
        {orderedSections ? (
          orderedSections.map((section, index) => (
            <LinkSection
              ...
              isCollection={section.type === "COLLECTION"}
